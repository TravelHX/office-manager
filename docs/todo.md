...
- [ ] 23.13 Integration tests: map config endpoints (admin auth, file upload), undo cancel happy path and expiry, booking list still works without map if no image configured (Phase 23c portion delivered: `tests/integration/undo-cancel.test.js` — 8 tests covering happy path, expired window, taken desk, admin-cancelled rejection, wrong-user rejection, 404, 400 not-cancelled, and `X-Undo-Window-Ms` header; map-config portions remain for Phase 23d/e)
- [ ] 23.14 Frontend Jest tests: map rendering with mock config, button visibility when selected, undo UI (Phase 23b button-visibility already delivered; Phase 23c undo UI delivered: `src/frontend/tests/undo-cancel.test.js` — 6 tests covering toast render, auto-dismiss timer, replace-on-second-cancel, Undo click success + failure, mid-flight disable/relabel; map Jest remains for Phase 23e)
- [ ] 23.15 Playwright end-to-end test: admin uploads plan and places landmark; user sees map on desk or parking page; user cancels desk and undoes within window (Phase 23c delivered: `tests/e2e/undo-cancel.spec.js` — user seeds booking via API, opens My Bookings, cancels, clicks Undo in the toast, confirms booking row returns to active; map portions remain for Phase 23e)
- [ ] 23.16 Update `docs/usecases.md`: add map orientation and undo flows; update multi-select manual paths for buttons (Phase 23c portion delivered: new **Use Case 12: User Undoes a Recent Desk Booking Cancellation** with description, steps, expected result, manual test path, and cross-references to the four test files; map/orientation flows remain for Phase 23e)
- [ ] 23.17 Update root `README.md`: document maps, undo, button behavior (Phase 23c portion delivered: Desk Booking feature list now mentions Undo; new **Undoing a cancellation** subsection under Desk Booking user guide; Implementation Summary gained Phase 23a / 23b / 23c lines; map docs remain for Phase 23e)
- [ ] 23.18 Update `docs/spec.md` **Currently Implemented** and API lists after delivery (Phase 23c portion delivered: section 18 annotated **Status: Implemented (Phase 23c)** with concrete window, error codes, and audit event; Desk Bookings API list gained `POST /api/bookings/:id/undo-cancel`, `POST /api/bookings/bulk`, and documents the `X-Undo-Window-Ms` response header; map-related API entries remain for Phase 23d/e)
